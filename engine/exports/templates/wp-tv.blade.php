...
<channel>
    ...
    @for ($i = 1; $i <= $data['page']; $i++)
        @foreach ($tmdb->getTVShows($data['sort'], $i, ['language' => $data['lang']])->getResults() as $items)
            @php
                $title = __('seo.tv_title', ['title' => title_case($items->getName())]);
                $backdate = is_null($data['backdate']) ? $items->getFirstAirDate() : $data['backdate'];
                $uniqTime = rand(strtotime($backdate), time());
                $pubDate = date( 'D, d M Y H:i:s', $uniqTime )." +0000";
                $postDate = date( 'Y-m-d H:i:s', $uniqTime );
                $postID = $items->getID();
                $slug = str_slug($items->getName());

                $content = view('exports.templates.post-tv', compact('items'));
            @endphp
            <item>
